...
## 3.스레드 간의 데이터 공유
    ...
    * 이를 방지하기 위해 POSIX 표준에서는 다양한 동기화 장치와 데이터 공유 방법을 지원한다.
    ...
    7. 조건 변수
		...
		* ------------------ 보충 자료 ------------------
		1. pthread_detach
			- 제어권을 자신이 가짐. 종료시 자원 해제를 스스로 마침.
			- thread 를 생성자가 종료되면 detach 된 thread 역시 종료된다.
		2. union 타입
			- 변수를 동싱 사용하지 않고 베타적으로 사용하는 경우 사용.
		3. static 타입 c 와 c++ 에서의 차이점
			* static
				- 내부 연결을 가진다. ( 외부에서 접근할 수 없다. )
				- 객체나 변수가 프로그램이 시작할 때 할당되고, 프로그램이 종료될 때 할당이 해제된다.
			* c static
				- 파일 내에서만 접근 가능하다.
			* c++ static
				- class 에서 사용하였을 때, class 의 모든 객체가 쓸 수 있는 변수/객체가 된다.
		4. cond-wait 은 왜 lock 과 함께 써야하나
			- 원자적으로 뮤텍스의 잠금을 해제하고 쓰레드를 블록시키도록 보장하기 위해서.
