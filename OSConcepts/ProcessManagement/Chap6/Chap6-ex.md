...
기아 현상을 유발하지 않는 readers-wirters 문제의 해결책을 제시하시오.
	- Reader-Writer 문제에서 공평성과 처리량 간의 trade-off
		- 
	- 기아 현상을 유발하지 않는 해결책
		- 만약 Readers 가 lock 을 잡고 있을 때 Writer 가 lock 을 요청한다면, 새로운 Reader 의 lock 요청은 대기 시키고 
		기존 Readers 가 lock 을 전부 해제하면 Writer 가 lock 을 획득한다. 
		- Writer 가 lock 을 잡고 있을 때 역시 마찬가지며, 이 방법을 사용하면 Reader 와 Writer 모두 기아현상을 방지할 수 있다.
